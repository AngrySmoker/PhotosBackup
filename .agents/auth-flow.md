...
4. **In-app capture:** the token is read in-process from the app's own non-persistent WKWebView cookie store — no extension, no App Group, no custom-scheme handoff. The web session is discarded on capture. (The old extension + `photosbackup://` handoff is retired; `HandoffStore` and the URL scheme are vestigial pending removal.)
